...
export const AIDiagnostics: React.FC<AIDiagnosticsProps> = ({ onBookWithIssue }) => {
  ...
  const [activeDiagnosis, setActiveDiagnosis] = useState<{
    ...
    recommendedPackage: 'Motor Controller & Throttle Fix (₹850)',
  });
    ...
    setTimeout(() => {
      ...
      if (q.toUpperCase().includes('E-07') || q.toLowerCase().includes('battery') || q.toLowerCase().includes('heat')) {
        setActiveDiagnosis({
          ...
          recommendedPackage: 'Battery Diagnostics & BMS Calibration (₹499)',
        });
        ...
        setActiveDiagnosis({
          ...
          recommendedPackage: 'Hydraulic Brake Bleed & Pad Replace (₹650)',
        });
      } else {
        setActiveDiagnosis({
          code: 'SYS-DIAG',
          ...
          recommendedPackage: 'Full 30-Point Safety & Torque Tune (₹999)',
        });
      }
    }, 800);
  };
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600">Instant AI fault scanner for electric bikes, scooters & cargo EVs.</p>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            <Search className="w-5 h-5 absolute left-3.5 top-3.5 text-red-600" />
            <input
              type="text"
              value={query}
              ...
              className="w-full bg-slate-50 border border-slate-300 focus:border-red-600 rounded-2xl pl-11 pr-4 py-3.5 text-sm text-black placeholder-slate-400 focus:outline-none transition-colors"
            />
          </div>
          <button
            onClick={() => handleAnalyze()}
            disabled={isAnalyzing}
            className="px-6 py-3.5 bg-red-600 hover:bg-red-700 text-white font-black uppercase text-xs tracking-wider rounded-2xl transition-all shadow-md active:scale-95 shrink-0 flex items-center gap-2"
          >
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pt-1">
          ...
          {commonFaults.map((f) => (
            <button
              ...
              className="px-3 py-1 bg-slate-100 hover:bg-red-50 text-slate-700 hover:text-red-700 border border-slate-300 hover:border-red-300 rounded-full text-[11px] font-mono transition-colors shrink-0 font-medium"
            >
              {f.code}: {f.title}
            </button>
          ))}
      ...
      {activeDiagnosis && (
        <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm relative overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-200 pb-4">
            ...
            <span className="text-xs text-slate-500 font-mono font-bold">99.2% Diagnostic Accuracy</span>
          </div>

          <div className="space-y-4">
            <div>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Identified Symptom</p>
              <p className="text-xl font-black text-black mt-1">{activeDiagnosis.symptom}</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-1">
                <p className="text-xs font-bold text-slate-800 uppercase flex items-center gap-1.5">
                  <AlertTriangle className="w-4 h-4 text-amber-600" /> Root Cause Analysis
                </p>
                <p className="text-xs text-slate-600 leading-relaxed font-medium">{activeDiagnosis.cause}</p>
              </div>

              <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-1">
                <p className="text-xs font-bold text-slate-800 uppercase flex items-center gap-1.5">
                  <CheckCircle2 className="w-4 h-4 text-emerald-600" /> Immediate Rider Action
                </p>
                <p className="text-xs text-slate-600 leading-relaxed font-medium">{activeDiagnosis.immediateFix}</p>
              </div>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm font-black text-red-600">{activeDiagnosis.recommendedPackage}</p>
            </div>

            <button
              onClick={() => onBookWithIssue(activeDiagnosis.recommendedPackage)}
              className="w-full sm:w-auto px-6 py-3.5 bg-red-600 hover:bg-red-700 text-white font-black uppercase text-xs tracking-wider rounded-xl transition-all shadow-md active:scale-95 flex items-center justify-center gap-2"
            >
