...
const SAMPLE_PRESETS = [
  ...
  },
  {
    ...
    prompt: 'Close-up slow motion shot of an electric vehicle lithium battery being slotted into a smart swap station with glowing indicator lights.',
  }
];
...
export const AIGenTab: React.FC<AIGenTabProps> = ({ onOpenBooking }) => {
  ...
  const handleGenerateVideo = async () => {
    if (!selectedImage) {
      setErrorMessage('Please choose or upload a source image first.');
      return;
    }

    if (!prompt.trim()) {
      setErrorMessage('Please enter a motion prompt describing the animation.');
      return;
    }
    ...
    setStatusMessage('Initiating Veo 3.1 video generation...');
    ...
    try {
      // Step 1: Request Veo generation via server proxy
      ...
      if (!startRes.ok || startData.error) {
        throw new Error(startData.error || 'Failed to trigger video generation');
      }

      const operationName = startData.operationName;
      if (!operationName) {
        throw new Error('No operation tracking ID received from server');
      }

      setGenerationProgress(15);
      setStatusMessage('Veo AI neural rendering started... Processing frames.');
      ...
      pollingIntervalRef.current = setInterval(async () => {
        attempts++;
        const simulatedProgress = Math.min(92, 15 + Math.floor(attempts * 4));
  ...
  return (
    <div className="space-y-8 animate-fadeIn max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 text-slate-800">
      {/* Top Banner Header */}
      <div className="relative rounded-3xl p-6 sm:p-8 bg-white border border-slate-200 overflow-hidden shadow-sm">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="px-3 py-1 bg-red-50 text-red-600 border border-red-200 text-xs font-black uppercase tracking-widest rounded-full flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-red-600" />
                VEO 3.1 FAST ENGINE
              </span>
              <span className="px-2.5 py-0.5 bg-slate-100 text-slate-700 text-[10px] font-mono font-bold rounded border border-slate-200 uppercase">
                IMAGE TO VIDEO
              </span>
            </div>
            ...
            <p className="text-sm text-slate-600 max-w-2xl font-medium">
              Upload any electric bike, scooter, workshop, or road photo and let Google's flagship <strong className="text-red-600 font-mono">veo-3.1-fast-generate-preview</strong> model transform still images into fluid motion videos.
            </p>
          </div>

          <div className="flex items-center gap-3">
            <div className="px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl flex items-center gap-3">
              <Film className="w-6 h-6 text-red-600" />
              <div>
                <p className="text-[10px] uppercase font-bold text-slate-500">Supported Output</p>
                <p className="text-xs font-mono font-bold text-black">16:9 Landscape & 9:16 Reels</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Error Alert */}
      {errorMessage && (
        <div className="p-4 bg-red-50 border border-red-200 rounded-2xl text-sm text-red-700 flex items-start gap-3 animate-fadeIn">
          ...
          <button
            ...
            className="text-xs text-red-700 hover:text-black underline font-bold"
          >
            Dismiss
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="p-6 bg-white border border-slate-200 rounded-3xl space-y-5 shadow-sm">
            ...
            <div
              ...
              className={`relative rounded-2xl border-2 border-dashed transition-all cursor-pointer overflow-hidden flex flex-col items-center justify-center group ${
                selectedImage
                  ? 'border-red-300 bg-slate-50 min-h-[260px]'
                  : 'border-slate-300 hover:border-red-500 bg-slate-50 hover:bg-slate-100 min-h-[220px]'
              }`}
            >
              <input
                ref={fileInputRef}
              ...
              {selectedImage ? (
                <div className="relative w-full h-full min-h-[260px] flex items-center justify-center bg-slate-100">
                  <img
                    src={selectedImage}
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 p-4">
                    ...
                    <span className="text-xs font-bold text-black bg-white px-3 py-1.5 rounded-xl shadow-md">
                      Click or Drop to Replace Image
                    </span>
                  </div>
                  <span className="absolute bottom-3 left-3 bg-white/90 backdrop-blur-md px-3 py-1 rounded-lg text-xs font-mono text-black font-bold border border-slate-200 shadow-sm">
                    {selectedImageName}
                  </span>
                </div>
              ) : (
                <div className="p-8 text-center space-y-3">
                  <div className="w-14 h-14 mx-auto rounded-2xl bg-red-50 border border-red-200 flex items-center justify-center text-red-600 group-hover:scale-110 transition-transform">
                    <Upload className="w-7 h-7" />
                  </div>
                  <div>
                    <p className="text-sm font-bold text-black">Drop your bike or scooter photo here</p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                {SAMPLE_PRESETS.map((preset, idx) => (
                  ...
                  >
                    ...
                    />
                    <p className="text-[10px] font-bold text-slate-800 mt-1.5 truncate px-1">
                      {preset.title}
                    </p>
                  </button>
                ))}
          ...
          <div className="p-6 bg-white border border-slate-200 rounded-3xl space-y-5 shadow-sm">
            ...
            {/* Prompt Input */}
            <div>
              <label className="text-[11px] uppercase font-bold text-slate-600 block mb-2">
              ...
              <textarea
                ...
                rows={3}
                className="w-full bg-slate-50 border border-slate-300 focus:border-red-600 rounded-2xl p-4 text-xs text-black placeholder:text-slate-400 focus:outline-none transition-colors"
              />
            </div>
            ...
            <button
              ...
              className={`w-full py-4 rounded-2xl font-black text-sm uppercase tracking-widest transition-all flex items-center justify-center gap-3 shadow-md active:scale-98 ${
          ...
          <div className="p-6 bg-white border border-slate-200 rounded-3xl space-y-5 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              {currentGeneration?.status === 'completed' && (
                <span className="px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-mono font-bold rounded uppercase flex items-center gap-1">
            ...
            {/* Video Display Area */}
            <div className={`relative rounded-2xl bg-black overflow-hidden border border-slate-200 flex items-center justify-center ${
              ...
              {isGenerating ? (
                <div className="p-6 text-center space-y-4 max-w-sm">
                  <div className="relative w-16 h-16 mx-auto">
                    ...
                    <div className="absolute inset-0 rounded-full border-4 border-t-red-600 border-r-red-400 animate-spin"></div>
                    <Sparkles className="w-7 h-7 text-red-500 absolute inset-0 m-auto animate-pulse" />
                  </div>
                  ...
                  <span className="text-xs font-mono font-bold text-red-400">{generationProgress}% Completed</span>
                </div>
                ...
                <div className="p-8 text-center space-y-3 text-slate-400">
                  ...
                  <p className="text-xs text-slate-300">No video generated yet.</p>
            ...
            {(activePlaybackVideo || currentGeneration?.videoUrl) && (
              <div className="flex items-center justify-between gap-3 pt-2">
                <a
                  ...
                  className="flex-1 py-3 px-4 bg-red-600 hover:bg-red-700 text-white rounded-xl text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-2 shadow-sm"
                >
          ...
          <div className="p-6 bg-white border border-slate-200 rounded-3xl space-y-4 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              {videoHistory.length > 0 && (
                <button
                  onClick={() => saveHistory([])}
                  className="text-[10px] text-slate-500 hover:text-red-600 uppercase font-bold transition-colors"
                >
                  Clear All
                </button>
              )}
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      {item.sourceImage ? (
                        ...
                        <div className="w-12 h-12 rounded-xl bg-red-50 flex items-center justify-center shrink-0 border border-red-200">
                          <Film className="w-5 h-5 text-red-600" />
                        </div>
                      )}
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-[9px] font-mono font-bold uppercase px-1.5 py-0.2 bg-red-100 text-red-700 rounded">
                            {item.aspectRatio}
                          </span>
                          <span className="text-[10px] text-slate-500">{item.createdAt}</span>
                        </div>
                        <p className="text-xs font-bold text-black truncate mt-0.5">{item.prompt}</p>
                      </div>
                    </div>
