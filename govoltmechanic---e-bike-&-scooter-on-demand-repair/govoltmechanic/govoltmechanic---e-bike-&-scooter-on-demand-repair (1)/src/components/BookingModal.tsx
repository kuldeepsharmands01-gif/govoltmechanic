...
export const BookingModal: React.FC<BookingModalProps> = ({
  ...
  const [address, setAddress] = useState('1240 Market St, Suite 400, San Francisco, CA');
  ...
  const [customerPhone, setCustomerPhone] = useState('+1 (555) 392-1084');
  ...
  const handleDetectLocation = () => {
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          setAddress(`GPS (${pos.coords.latitude.toFixed(4)}, ${pos.coords.longitude.toFixed(4)}) - Mission District, San Francisco`);
          setIsLocating(false);
        },
        () => {
          setAddress('742 Valencia St, Mission District, San Francisco, CA');
          setIsLocating(false);
        }
      );
    } else {
